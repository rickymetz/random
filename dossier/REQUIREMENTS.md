...
| Form rules (labels and inline controls) | An inline dropdown sits at the right edge of its row, label at the left. A label’s description renders inline in parentheses; once that would make the label wrap to two or more lines, the description drops to its own line in small subtle text (`LabelText` measures live, so the same label can be inline on a tablet and stacked on a phone) |
