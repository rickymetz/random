...
| The relationship vocabulary is editable too (§8.2) | Ties point at their type by id, so renaming one lands on every tie at once — “coworker” becomes “colleague” everywhere, including the chips already on a dossier — and the editor says how many ties a rename will touch before you make it. A type’s hue family is editable with its name. Deleting follows the same rule as the person form: a type in use retires (existing ties keep it and keep drawing, it stops being offered for new ones, Restore puts it back), and a type nothing uses is deleted outright — the button says which of the two it will do. “mentioned” is not vocabulary and is not listed: the note parser owns those edges. Covered in `smoke25` |
| The person form is the vault’s own (§8.1) | What one person needs on a dossier — allergies, gift ideas, what helps on a bad day — is noise to the next, so the form is subjective to whoever keeps it. The built-ins keep their typed slots, because birthday reminders, contacts import, name detection and the quiet lens read them by name; what the user invents lives beside them in a `custom` bag keyed by field id, with the definitions as their own record kind. Seven types: short text, long text, list of tags, date, pick one, number, yes or no. A field id is a uuid and never changes, so renaming is a label change and never touches an answer, and the type is fixed after creation — there is no honest way to turn a list of allergies into a date. Three starter packs (Family & friends, Work & professional, Just the essentials) give a first form to edit rather than a blank page, and running one twice adds nothing. Retiring is how a row leaves: off the form and off every dossier, answers untouched, Restore brings both back — tidying a form must never be the thing that loses what you typed — and deleting the answers is a second, explicit act that names how many it will destroy. One form for everyone, with only answered rows drawn, so a work contact you know three things about still reads as a short page. A fresh vault is asked on the home screen — “what do you want to remember about people?” with the three sets and a Not now — because a blank page in Settings is one nobody would think to look for; either answer settles it for good (`formSetupDone`, device-local, so a restore doesn’t bring it back — and wouldn’t show anyway, since a restored vault arrives with its form). Quick-add stays one field; the form is the Edit page. Search indexes the answers and stops the moment a row retires; list answers become facets like tags; a date row can ask for the same yearly reminder a birthday gets, with its own lead time, and shows in Upcoming beside them; a restore matches rows by name so two “Allergies” never split their answers. The sanitizer runs on every unlock and has eaten a field before, so every shape round-trips in a test — including the two that look like absence and are not, a “no” and a zero. `smoke25` walks pack → own row → answer → dossier → search → retire → restore |
| Review pass on the graph and passkey round | Six findings from a review of the branch, all fixed before merge. A filter has to be visible to be undoable: circle narrowing now reads the rail’s own pool (in an ego view, the circles with someone in scope), so a circle hidden in one view can no longer thin another from off-stage with no chip and no share of the “N hidden” count — it was taking an ego view from 5 people to 3 with nothing on screen to say why. A highlighted path outranks the filters person by person, the way its edges already did, so the breadcrumb never names a step that isn’t drawn. A hidden self only counts as hidden while there is a “me” to hide (un-marking “This is me” used to leave a permanent “1 hidden” with no chip). The camera re-fits on a tie-type tap only in an ego view, where the walk follows edges and the tap really does take people off the screen. On the passkey side: only AES-GCM refusing the wrap (`OperationError`) retires an enrollment — every other failure is the platform having a bad moment and must not cost a passkey that still works — and the retire deletes the credential that failed rather than clearing the table, so one bad credential can’t take a good one with it; an authenticator answering with a credential that isn’t ours is a platform oddity, not a passkey to retire; and “no enrollment at all” (a flag gone stale) takes the button away quietly instead of accusing a passkey that was never there. `smoke22` and `smoke23` cover the four behavioural ones, each confirmed to fail against the code as reviewed |
| A `me` lens on the graph | You are joined to everyone, so the dot in the middle is the one that tells you least and pulls the layout hardest. A `me` chip on the rail (gold dot, beside the other lenses, on by default, remembered per session like them) takes you out and leaves your people and the ties between *them* — the shape you can’t otherwise see: on the sample cast, 15 people and 23 links become 14 and 19, the three circles separate, and the camera re-fits because the set of people changed. An ego view still starts from you — it is your neighbourhood either way — you just aren’t drawn in it; a highlighted path brings you back, because it starts at you and half a path is a lie, the same rule the path’s own edges already follow. Hiding yourself counts toward “N hidden · Show all”, so a vault holding only you can’t end up a blank canvas. Covered in `smoke22` |
| Discs stop at the tap target | A person’s disc grows with the zoom only until it is 44px across — the iOS minimum interaction size — and then holds (`MAX_NODE_SCREEN_R`, read through `radiusOf` in the render). Past that point zooming adds space between people instead of size to each one: the layout keeps its world distances, so the gaps open on screen while the discs stay a thumb wide (measured: at the fit zoom a neighbour is 61px away and a disc 43px across; four zoom steps later the disc is unchanged and the neighbour is 240px away, where it used to reach 101px across). It also means that once you are close enough to read names, everyone is a proper target rather than only the hubs — size-by-degree still reads at the zooms where the whole shape is the subject, and stops mattering when one person is. The ring, the focus ring, the photo crop, the selected halo, the pin tick, the arrowheads, the name’s offset and the boxes names dodge all read the same clamped radius, so they stop together; the initials ride it too, holding at the screen size they had reached. Hit testing follows the disc you can see, never smaller than a finger. `smoke24` measures the painted run through a person’s centre and the distance to their nearest neighbour at three zooms — confirmed to fail against the unclamped build |
| The mention picker can’t index a list that shrank | The highlighted suggestion is state, and the effect that resets it runs after render, so a keystroke landing in that window reads a list that has already got shorter: arrow down to the third match, keep typing until one is left, press Enter, and the note editor threw `Cannot read properties of undefined (reading 'kind')` — seen once as an intermittent `smoke9` failure on CI's slower runner, never reproduced locally even at 40× CPU throttling. The index is now clamped where it is read, so the window cannot exist, the arrows count from the clamped value, and `pick` ignores a hole |
...
| A newcomer adding people in bulk (five personas) | A first-time layperson, an iOS interaction designer, a data-migration hand, a VoiceOver user and a privacy-minded skeptic each walked the two ways in — pasting a list into “Add several” and importing a phone’s .vcf — on a fresh vault at phone size. Speed was never the problem (300 names preview in full, a keystroke costs 84 ms at 6× CPU throttling, 375 contacts parse in 54 ms and write in 151 ms); what you were left holding was. Fixed: **where you land.** Both bulk toggles sit at the *tail* of the list, so restoring focus to the one you came from — right for Cancel — scrolled a newcomer who had just imported 375 people to y = 3,842, the bottom of a list whose top they had never seen, where the paging observer then loaded more and pushed them further. Adding now lands on the “All people 375” heading at the top instead (`tabIndex=-1`, so it reads as that to a screen reader, which otherwise heard only “Import contacts, button”) — but only once the list is long enough to have that heading: on a short one the toggle never left the screen, so cancelling and adding both go back to it. **The graph.** A vault past the label limit opens on your own connections rather than a hairball — but straight after an import nobody is connected to anybody, so that drew one lone gold disc under “Tap someone to see who they know” with no one to tap, and suppressed the “No relationships yet” message written for exactly this moment (the auto-focus made `bare` false). The ego default now waits until you yourself are joined to somebody — circles don’t count, since an ego view walks edges and membership isn’t one — and the message counts the people waiting: “No connections yet — all 375 people are here, but nothing joins them up.” **Undo.** The contacts import and “Looks like people” both let you take a bulk add back; “Add several” did not, so 300 pasted names came out one delete at a time. It now offers Undo beside Done — on the People page only, where the submit creates people and nothing else: inside a dossier the same submit also writes ties and clears the mention edges they replace, and half an undo is worse than none. **Recent.** The row pads out with the most recently updated people when there are no visits yet, which is right on a restored device and wrong after an import, where it headed the home screen with six strangers truncated to “Ada A., Ada B., Ada B., Ada C.” — the padding is now limited to people who have actually been worked on (`updatedAt > createdAt`), so the row stays away until you open someone — a restored backup still fills it, because those people carry months of edits. `smoke13`’s “always six” became “nothing before a visit, then what you visited”. **The first-run form offer** kept a screenful between Recent and the list once the vault held hundreds; past eight people it shrinks to one line with the same Not now. **Namesakes inside one file.** Every phone export carries the same person synced from two accounts; the copy that adds nothing was already dropped, but the one that disagreed on e-mail or phone became a second person silently — the `conflict` flag that exists for precisely this ambiguity only ever looked at the vault. A second card with a name already used in the file is now tagged “same name, earlier in this file”, counted in the summary, and skipped by “Select all new” (as vault-side conflicts now are too): a namesake is a decision, not a sweep. **The paste preview** badged all 300 rows “new”, which is noise when they all are — the badges mark the exceptions now, and a mixed paste gets the import panel’s summary line instead. Not taken: guessing which contacts are businesses (“Ace Plumbing”, “no-reply@bank.example” and “Plumber” all import as people — the panel already puts you in control of who comes, and the heuristics for the rest are a minefield), and a bulk delete from the list. `smoke26` walks the whole journey, both ways in |
| A value that outgrows its box | A pick-one row’s choices were one comma string in a single-line field: past three or four words you were reading and editing a list through a letterbox, and a choice could never contain a comma. They are chips now — the same move tags, likes and nicknames made, for the same reason — so the whole list wraps into view and one × takes one choice off. A typed comma still commits, so the habit the old field taught keeps working. Fixing it turned up two real bugs underneath. **The word still in the field was lost on Save.** A chip field dropped its half-typed value whenever the blur went to any button, to avoid committing one on the way to Cancel — but Save is a button too, so “karaoke” typed and then saved simply vanished, on every chip field in the app. **And the tap itself could vanish.** Committing on blur adds a chip, the chip wraps onto a new line, the field grows by a row, and the button under your finger moves before you lift it: measured at 33px of drift, and Chrome delivers no click at all — you tap Save and nothing happens. Both are fixed by the same rule: a chip field never commits on its way to a button (the layout holds still, the tap lands) and instead reports what is still being typed, which the form folds in on save (`withDraft`, trimmed and deduped like any other commit). Covers the five built-in chip fields, the custom list rows and both Choices editors. Separately, the writing boxes now take the height of what is in them and stop at a ceiling (`useGrow`): a long “How we met” or a long-text answer read through two rows was the same letterbox, and the note box grows to six lines above the keyboard so Save note stays in reach. The ceiling is measured from the computed line height, not a pixel guess, so it holds at 200% text; only the ceiling is the hook’s — a floor belongs to the stylesheet, which may be animating it (the capture bar eases open on focus). `smoke27` walks it: chips from a comma-typed list, the leftover word kept by Add row, one × removing one choice, the dossier picker offering exactly those, and both boxes growing without pushing their buttons off the screen |
| Visual bugs from a phone review | Four, each traced to a cause before it was fixed. **Edges ran through names.** A name’s halo was a 3px stroke hugging its letters, so a line passing between two of them showed straight through (“Alex K|im”, “Del|gado”, an arrowhead on “Me (you)”). Names and circle names are now placed before a single edge is drawn, and the edges and arrowheads are clipped around every label box — a map’s knock-out, so a line stops at a name and resumes after it, while bubbles and discs are untouched. Painting order is unchanged: names still go on last. **Circle names wandered off.** The placer tried the top corners, then below, then climbed up to seven lines away; on a phone WEBB FAMILY settled far above its bubble with an edge through it, MERIDIAN LABS landed beside Grace Liu (who isn’t in it) and one bubble went unnamed. A circle’s name is now chosen only from points on its own rim (`lib/hullLabel.ts`: anchors round every member at the bubble’s padding, over the top first, then the sides, then below), sliding along the point or stepping out at most one line, with a clear margin from people and their names and a preference against sitting in another bubble. **“Restore” broke mid-word** into “Restor / e”: the Dynamic Type rule gave every button and chip `overflow-wrap: anywhere`, which also shrinks their minimum width to one letter, so a flex row squeezed the button instead of the field. It is `break-word` now — the longest word stays whole and only a word that could never fit is split — and the restore row lets its button drop below the field at large text instead. The backup picker was the browser’s own control, a second bordered button inside our field with its own lowercase “no file selected”; it is our button over a hidden input, naming the chosen file, as Import contacts does. **The graph’s view buttons** were Unicode glyphs, and a glyph is whatever the system font makes of it: on iOS “⤢” came out half the size of its neighbours. They are drawn icons in the tab bar’s stroke. `smoke28` checks the button stays one word (and at 200% text), the picker, that every sample bubble is named within a line of its own rim and never over a person’s name, and that the view buttons are icons |
| Nothing shows under the tab bar | A phone screenshot of the installed app showed the tab bar floating about 50pt above the screen’s bottom over a strip of page background. Nothing in the app puts it there — the bar is `position: fixed; bottom: 0`, no ancestor makes a containing block, and it already carries the home-indicator inset — so iOS anchored fixed elements short of the screen it drew, which a headless browser can’t reproduce. The bar now owns everything below it: a pseudo-element paints the bar’s own background 50vh downward, so where iOS leaves a strip it reads as more bar instead of a band, and where iOS gets the bottom right it sits below the screen unseen (a fixed element’s overflow is not scrollable, so no page gains anything to scroll into). The underlying anchoring is still iOS’s; if the labels ever sit visibly high, that is where to look. Found on the way: `--appbar-h` was the literal 3.25rem (52px) while the bar holds a 44px control and is 61px tall, so the graph page scrolled 9px under a canvas that can’t scroll it back, sticky A–Z letter headers tucked 9px under the bar, and sheet heights were 9px generous. It is derived now — the control, its padding and the hairline — so it can’t go stale again. `smoke28` lifts the bar 50px to stand in for iOS and reads the strip’s colour, and checks the graph page doesn’t scroll; each check confirmed to fail without its fix |
| Check for updates without reinstalling | An installed copy already updated itself — `autoUpdate` service worker, an hourly check while open, and a reload onto the new build once it takes over — but it could not answer “a fix just shipped: do I have it?” on demand, so the only sure way was to delete the app from the Home Screen and add it back. Settings → Updates says which build is running (`0.1.0 (ff41ad7b), built 22 Sep 2026` — version from package.json, commit from CI’s SHA or the checkout, stamped at build) and has **Check for updates**, which asks the server now (`registration.update()`, past the HTTP cache) and says what it found in words: the latest version; a new one installing, after which the app reopens on it *locked*, as any reload leaves it; or no connection — a device that knows it is offline is told so rather than “latest”. If the new build hasn’t taken over within 20 seconds, a Reopen now button appears instead of leaving “installing” on screen. The section never names the app, so a disguised copy doesn’t say “Ledger”. Where there is no service worker (development, or a browser without one) the button is hidden and the line says the browser loads the latest version on open. `smoke29` serves a private copy of the build, checks (nothing new), ships a “release” by changing that copy’s worker on disk, checks again, and requires the app to find it, reopen onto it locked, run the new worker, and report offline as offline. (The background half changed straight after — next row.) |
| Updates wait for a quiet moment; notes being written survive a reload | The hourly background check used to reload the page the moment a new build took over (`autoUpdate`), wherever you were: it locked you out mid-sentence, and a reload runs no lock and flushes nothing, so a half-written note in the note box was simply gone. Two changes. **Updates wait.** The service worker is registered in `prompt` mode: a new build installs and waits, and App.tsx takes it only when nothing is in use — the moment the app locks, or on the unlock screen before anything has been typed there — never while unlocked. A relaunch after iOS closes the app picks it up too, since a waiting build activates when no page holds it. Settings → Updates says when one is waiting (“installs the next time the app locks — or now”) and Install now / Check for updates take it at once. The page reloads on `controllerchange` itself: the plugin reloads only for an update it counts as its own, and one found by `registration.update()` — the hourly check, the button — is “external” to it, which left the page on the old build under the new worker (measured before fixing). `clientsClaim` keeps the first install in charge of the page it came from, as before; waiting stays the app’s call. Prompt mode is also the safer one for lazily loaded pages: the old worker keeps serving the old build’s chunks until the switch. **Drafts are kept on disk.** The note box writes what you are typing as a `draft` record — encrypted like every record — after a 600ms pause, at once when the page is hidden or left, and forgets it at once when emptied; unlock puts it back in the box. Drafts never join the in-memory records the app renders from (they are split off at unlock), which is what keeps them out of backups — a first version kept them in the Settings record, which the backup writes out — and what keeps a pause in typing from handing every page a new records map to re-render. Saving the note and forgetting its draft are one write (`saveNote(…, { clearDraft })`), on Save and when a lock flushes the box, so a reload or a lock that stops waiting can’t leave the text as both. A restore ignores any draft a bundle claims to carry; deleting a person deletes their draft; the sanitizer keeps only a person id and text capped like a note. A review before merging also tightened the rest: an explicit “update now” lapses after a minute, so a request whose build failed to install can’t reload the page mid-sentence when the next one arrives hours later; Reopen now takes a waiting build rather than reloading onto the old one; a chip field clears its leftover on the way out (Cancel no longer lets an invisible word reach the next save) and reports it in the vocabulary’s spelling; growing boxes refit when their width changes; circle names skip bubbles that are off screen and search one pass under a budget shared across the bubbles in view (a few get a near-exhaustive search, a crowd a floor each). On a 1,000-person vault panned for 3 s at 1× zoom the page now spends 2.6–2.7 s in script against 2.5 s for the code before this change and 3.1 s for the unbounded rim search; the remaining ~5% is the label planning and edge clipping that keep lines out of names, not the circle search (halving its budget changed nothing measurable). Not taken: dropping a one-letter leftover on Save (it is on screen when Save is tapped, and a pick-one row may well hold “5”). `smoke29` covers it end to end against a private copy of the build: on request an update reopens the app onto it; found in the background while a note is half-written it installs and waits with no reload and the text untouched, then installs at the lock with the draft kept as a note; a reload nobody asked for leaves the text back in the box after unlocking. Confirmed to fail against the previous code, at the reload mid-note |
| Drafts in the note editor and the Edit form | The note box kept its text through a reload; the other two places you write did not. An edit to a note, or to a person’s details, left open when the app went away — an update applying itself, iOS closing the app, a lock — was simply gone, and on a phone the form was lost to a tab tap as well. Both now keep a `draft` record the same way (a `slot` on it says where: absent for the note box, `note:<id>` for a note being edited, `details` for the form), through one hook (`usePersistedDraft`) that the note box now uses too: written after a 600ms pause, at once when the page is hidden or left, forgotten at once when emptied. A lock asks every open editor to write what is still waiting on a pause (`registerDraftSaver`) before the key goes, so typing and locking in the same breath keeps it. An editor reads its draft back synchronously (`readDraft`), ahead of the queued write, so leaving for another tab and coming straight back finds it. **Coming back**: the page opens with the editor already open, what was typed in place, and a line saying so (“Your unsaved changes are back.” / “Your unsaved edit is back.”); a form that reopens itself leaves focus where the page puts it. The form’s draft is only the fields that changed (and any half-typed chip text), laid over what the person has now — so a draft never rolls back an edit made since — and anything that no longer fits (a field since removed, a value of the wrong shape) is dropped, never trusted (`lib/detailsDraft.ts`). **Letting go**: Save forgets the draft in the same write as the change (`updatePerson`/`updateNote` with `clearDraft`), and the text is blanked while saving so the saved record coming back tidied up (trimmed, relinked) never looks like changes left open; Cancel and Discard forget it at once. Deleting a note deletes its kept edit; deleting a person deletes every draft about them; an unlock throws away drafts whose note or person is gone. A form or edit draft is never a note: a lock keeps it as a draft to reopen, where the note box’s becomes a note as before. The sanitizer accepts only those slots, with a note id that is an id, and gives the form’s draft 200 KB. Accepted: forgetting is a write too, so a reload in the same instant as Cancel can still bring the edit back. `smoke30` covers it: the form reopens after a reload with the location and a half-typed tag, keeps a change typed and locked at once, and is gone once saved or discarded; a note edit reopens after a reload, survives a trip to another tab, and is gone once saved or cancelled. Confirmed to fail against the previous code |
