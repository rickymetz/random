...
| `games/*.js` | Minigames: `flap` (free-for-all), `sling` (teams), `chomp` (1 vs rest), `snipe` (Sniper Plaza and Sniper Blackout, 1 vs rest, one engine), `tag` (Infection), `kaiju` (Kaiju), `soccer`, `paint`, `crown`, `dodgeball`, `tug`, `relay`, `stack` and `bombsquad` (teams) and `sumo`, `bumper`, `coinrush`, `potato`, `snake`, `maze` (tilt) and `draw` (free-for-all) on the shared `arena` engine, plus `gauntlet` (the Microgame Gauntlet mode: 9 microgames, lives, speed-ups). Each has a bot. |
